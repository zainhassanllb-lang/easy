                          ...
                          selectedWorker.profileImage
                            ? (selectedWorker.profileImage.startsWith('http')
                              ...
                              : selectedWorker.profileImage.startsWith('/')
                                ? `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"}${selectedWorker.profileImage}`
                                : `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"}/uploads/workers/${selectedWorker.profileImage}`)
                            : "/placeholder.svg"
                        }
                          ...
                          <div className="flex flex-col gap-2">
                            {selectedWorker.cnicImages.map((img: string, idx: number) => {
                              const imageUrl = img.startsWith('http')
                                ? img
                                : img.startsWith('/')
                                  ? `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"}${img}`
                                  : `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"}/uploads/workers/${img}`
                              return (
                              ...
                              src={
                                selectedWorker.selfieImage.startsWith('http')
                                  ...
                                  : selectedWorker.selfieImage.startsWith('/')
                                    ? `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"}${selectedWorker.selfieImage}`
                                    : `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"}/uploads/workers/${selectedWorker.selfieImage}`
                              }
                              alt="Worker Selfie"
                              fill
                              className="object-cover"
